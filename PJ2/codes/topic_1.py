...
def train_model(model, optimizer, criterion, epochs=100, scheduler=None, experiment_name="base"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (data, target) in  enumerate(tqdm(train_loader, desc=f'Epoch {epoch+1}/{len(train_loader.dataset)}')):
...
def load_experiment_results(output_dir):
    """
    从输出目录加载所有实验结果并组织成适配generate_comprehensive_report的格式
    """
    # 初始化结果容器
    base_results = {}
    filter_results = []
    ...
    for filename in os.listdir(output_dir):
        if filename.startswith('performance_report_') and filename.endswith('.json'):
            ...
            if exp_name == 'base_model':
                base_results = {
                    'best_test_acc': report['test_accuracy'],
                    'params': report['total_parameters'],
                    'training_time': report['training_time']
                }
            elif exp_name.startswith('filters_'):
                config_name = exp_name.split('_')[1]
                ...
                activation_name = exp_name.split('_')[1]
                ...
                optimizer_name = exp_name.split('_')[1]
...
def main():
    ...
    # 打印最终结果
    print("\n=== 所有实验已完成 ===")
    print(f"基础模型准确率: {results['base_results']['best_test_acc']:.2f}%")
    best_acc = max([r['test_acc'] for r in results['optimizer_results']])
    print(f"最佳整体准确率: {best_acc:.2f}%")
...
#         },
#         {
#         "config": {'name': 'WeightedCrossEntropy'},
